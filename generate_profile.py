...
INFO = [
    ("__header__", "Anirudh Phophalia", ""),
    ("__rule__", "", ""),
    ("__section__", "~/about", ""),
    ("Quote", "💻 Crafting code by day, 🎏 flying kites by breeze", "val"),
    ("", "   and 🔢 solving math puzzles by night.", "val"),
    ("Motto", "Debug your limits, optimize your time,", "val"),
    ("", "   and let your ideas soar like kites.", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/role", ""),
    ("Role", "AI/ML Enthusiast & Research-Oriented Developer", "val"),
    ("Edu", "B.Tech CSE - Thapar Institute | B.S. Data Science - IIT Madras", "val"),
    ("Focus", "Cloud Computing · MLOps · Distributed Systems · AI Security", "accent"),
    ("__blank__", "", ""),
    ("__section__", "~/stack", ""),
    ("Languages", "Python · C++ · JAVA · C · JavaScript · TypeScript", "val"),
    ("Frontend", "Angular · React · Vue.js · TailwindCSS · Bootstrap · Material UI", "val"),
    ("Backend", "Flask · FastAPI · Node.js · NestJS · Express.js", "val"),
    ("Database", "PostgreSQL · MongoDB · MySQL · OracleDB · SQLite · Firebase", "val"),
    ("AI/ML", "TensorFlow · PyTorch · Scikit-Learn · OpenCV · Keras", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/experience", ""),
    ("Research", "Adversarial ML · Drone Detection · Semantic Segmentation", "warn"),
    ("__blank__", "", ""),
    ("__section__", "~/projects", ""),
    ("USV-Seg", "Semantic segmentation framework for USV perception", "warn"),
    ("InvoiceSnap", "AI-powered invoice processing & management", "warn"),
    ("MAD1-Quiz", "Interactive quiz platform with time-bound rounds", "warn"),
    ("QR-Gen", "Real-time QR code generator supporting logos", "warn"),
    ("__blank__", "", ""),
    ("__section__", "~/certifications", ""),
    ("Data Science", "B.S. Degree from IIT Madras", "val"),
    ("__blank__", "", ""),
    ("__section__", "~/reach", ""),
    ("GitHub", "github.com/AnirudhPhophalia", "accent"),
    ("LinkedIn", "linkedin.com/in/anirudh-phophalia", "accent"),
    ("Mail", "anirudhphophalia@gmail.com", "accent"),
    ("__blank__", "", ""),
    ("__section__", "~/stats", ""),
    ("__stats__", "", ""),
]
...
def fetch_stats():
    ...
    try:
        ...
        try:
            import re
            req = urllib.request.Request("https://komarev.com/ghpvc/?username=AnirudhPhophalia", headers=headers)
            svg_data = urllib.request.urlopen(req, timeout=10).read().decode('utf-8')
            matches = re.findall(r'>([\d,]+)</text>', svg_data)
            if matches:
                stats["visitors"] = matches[-1]
        except Exception as e:
            print(f"[warn] visitors fetch failed: {e}")
            stats["visitors"] = "-"
...
def render(theme_name, colors, stats, ist_now):
    ...
    for label, value, ckey in INFO:
        ...
        if label == "__header__":
            ...
            stat_txt = (f'repos {stats.get("repos", "-")}   ·   stars {stats.get("stars", "-")}'
                        f'   ·   followers {stats.get("followers", "-")}   ·   visitors {stats.get("visitors", "-")}')
            parts.append(
    ...
    parts.append(
        ...
        f'<tspan class="key">➜</tspan>'
        f'<tspan class="acc" dx="8">~</tspan>'
        f'<tspan class="val" dx="8"> AI Researcher &amp; Developer </tspan>'
